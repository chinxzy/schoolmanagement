<template>
  ...
  <div class="container px-20 py-10">
    ...
    <table
      v-if="success == true"
      class="table-auto w-full text-start mt-10 p-3 mb-40"
    >
      <thead class="bg-orange-700 text-white">
        <tr>
          <th class="text-start p-4">Subject</th>
          <th class="text-start p-4">Classtype Link</th>
        </tr>
      </thead>
      <tbody>
        <tr
...
import { Subject, useSubjectStore } from "../store/subject";

export default defineComponent({
  setup() {
    const subjects = useSubjectStore();

    const subjectItem = computed(() => subjects.$state.subjects.subjects);

    const loading = computed(() => subjects.$state.loading);

    const success = computed(() => subjects.$state.success);
